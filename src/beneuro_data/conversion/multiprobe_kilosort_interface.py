...
def add_one_probe_to_nwbfile(
    ...
    """
    Primary function for converting the data in a SortingExtractor to NWB format. This function
    is copied from neuroconv to add waveform means (templates) to the nwb file.
    ...
    if write_ecephys_metadata:
        sorting_interface.add_channel_metadata_to_nwb(nwbfile=nwbfile, metadata=metadata_copy)
        ...
        for unit_id in sorting_extractor.get_unit_ids():
            ...
            if metadata_column["name"] == "electrode_group" and nwbfile.electrode_groups:
                value = nwbfile.electrode_groups[
                    sorting_interface.sorting_extractor.get_unit_property(unit_id=unit_id, property_name="electrode_group")
                ]
